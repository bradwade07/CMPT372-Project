...
const helpers = {
  ...
  getProductIdByTags: async function (tags) { //TODO: fix
    try {
  ...
  getUserCartByUserEmail: async function (email) {//TODO: fix
    try {
      const query = `
        SELECT 
            product.product_id, 
  ...
  postUser: async function (street_name, city, province, post_code, country, user_email, type_id) {
    try {
      const address_id = await postAddress(street_name, city, province, post_code, country);
      await pool.query(`INSERT INTO userinfo (user_email, address_id) VALUES ($1, $2);`, [user_email, address_id]);
      await pool.query(`INSERT INTO user (user_email, type_id) VALUES ($1, $2);`, [user_email, type_id]);
    } catch (error) {
